...
defineTypeNameAndDebug(SWIMHTC, 0);

addToRunTimeSelectionTable(surfaceFilmRegionModel, SWIMHTC, mesh);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

wordList SWIMHTC::hsBoundaryTypes()
{
...
void SWIMHTC::resetPrimaryRegionSourceTerms()
{
    if (debug)
    {
        InfoInFunction << endl;
...
void SWIMHTC::correctThermoFields()
{
...
void SWIMHTC::correctHsForMappedT()
{
...
void SWIMHTC::updateSurfaceTemperatures()
{
    correctHsForMappedT();
...
void SWIMHTC::transferPrimaryRegionThermoFields()
{
    if (debug)
    {
        InfoInFunction << endl;
    ...
    wallHeatFlux_.correctBoundaryConditions();
...
void SWIMHTC::transferPrimaryRegionSourceFields()
{
    if (debug)
    {
        InfoInFunction << endl;
...
//    Info<<"min, max primaryEnergy "<<gMin(primaryEnergyTrans_)<<", "<<gMax(primaryEnergyTrans_)<<nl;
 ...
 tmp<fvScalarMatrix> SWIMHTC::q(volScalarField& hs) const
{
    return
    (
...
/*tmp<volScalarField> SWIMHTC::qq(volScalarField& hs) const
{
...
/*
    ...
    iceSp_ = chi / zna;*/
    volScalarField r = wallHeatFlux_; //htcs_->h() * (TPrimary_ - T_);
    forAll(r, i)
    {
	if (deltaIce_[i] < 0.0001)
	{
...
/*
    ...
    volScalarField aa=q(hs_) & hs_;
    Info<<"min, max q(hs_) & hs_ "<<gMin(aa)<<", "<<gMax(aa)<<nl;
    Info<<"min, max r "<<gMin(r)<<", "<<gMax(r)<<nl;
    volScalarField ab=radiation_->Shs();
    Info<<"min, max radiation_->Shs() "<<gMin(ab)<<", "<<gMax(ab)<<nl;
    volScalarField ac=rhoSp_ * hs_;
    Info<<"min, max rhoSp_ * hs_ "<<gMin(ac)<<", "<<gMax(ac)<<nl;
    Info<<"min, max rhoSp_ "<<gMin(rhoSp_)<<", "<<gMax(rhoSp_)<<nl;
    Info<<"min, max hs_ "<<gMin(hs_)<<", "<<gMax(hs_)<<nl;
    volScalarField ad=impSp_ * Cp_ * (TParcels_-Tf_);
    Info<<"min, max impSp_ * Cp_ * (TParcels_-Tf_) "<<gMin(ad)<<", "<<gMax(ad)<<nl;
    volScalarField zna = hs_ + Lf_;
    Info<<"min, max hs_+Lf_ "<<gMin(zna)<<", "<<gMax(zna)<<nl;
    Info<<"min, max iceSp_ "<<gMin(iceSp_)<<", "<<gMax(iceSp_)<<nl;
*/
    solve
    (
...
:
    ...
    (
        IOobject
        (
            "wallHeatFlux", // Same name as T on primary region to enable mapping
            time().timeName(),
            regionMesh(),
            IOobject::NO_READ,
            IOobject::NO_WRITE
        ...
        regionMesh(),
        dimensionedScalar(dimEnergy/dimTime/dimLength/dimLength, Zero),
...
const volScalarField& SWIMHTC::Cp() const
{
    return Cp_;
}


const volScalarField& SWIMHTC::kappa() const
{
    return kappa_;
}


const volScalarField& SWIMHTC::T() const
{
    return T_;
}


const volScalarField& SWIMHTC::Ts() const
{
    return Ts_;
}


const volScalarField& SWIMHTC::Tw() const
{
    return Tw_;
}


const volScalarField& SWIMHTC::hs() const
{
    return hs_;
}

const volScalarField& SWIMHTC::deltaIce() const
{
  return deltaIce_;
}
...
tmp<volScalarField::Internal> SWIMHTC::Srho() const
{
...
tmp<volScalarField::Internal> SWIMHTC::Srho
(
    const label i
) const
{
...
tmp<volScalarField::Internal> SWIMHTC::Sh() const
{
