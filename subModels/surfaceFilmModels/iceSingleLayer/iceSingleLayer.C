...
defineTypeNameAndDebug(iceSingleLayer, 0);

addToRunTimeSelectionTable(surfaceFilmRegionModel, iceSingleLayer, mesh);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

wordList iceSingleLayer::hsBoundaryTypes()
{
...
void iceSingleLayer::resetPrimaryRegionSourceTerms()
{
    if (debug)
    {
        InfoInFunction << endl;
...
void iceSingleLayer::correctThermoFields()
{
...
void iceSingleLayer::correctHsForMappedT()
{
...
void iceSingleLayer::updateSurfaceTemperatures()
{
    correctHsForMappedT();
...
void iceSingleLayer::transferPrimaryRegionThermoFields()
{
    if (debug)
    {
        InfoInFunction << endl;
...
void iceSingleLayer::transferPrimaryRegionSourceFields()
{
    if (debug)
    {
        InfoInFunction << endl;
...
void iceSingleLayer::correctAlpha()
{
    if (hydrophilic_)
    {
...
void iceSingleLayer::updateSubmodels()
{
    if (debug)
    {
        InfoInFunction << endl;
...
//    Info<<"min, max primaryEnergy "<<gMin(primaryEnergyTrans_)<<", "<<gMax(primaryEnergyTrans_)<<nl;
 ...
 tmp<fvScalarMatrix> iceSingleLayer::q(volScalarField& hs) const
{
    return
    (
...
/*tmp<volScalarField> iceSingleLayer::qq(volScalarField& hs) const
{
...
void iceSingleLayer::solveContinuity()
{
    if (debug)
    {
        InfoInFunction << endl;
...
void iceSingleLayer::solveThickness
(
...
void iceSingleLayer::updateDeltaIce()
{
/*
//    deltaIce_.correctBoundaryConditions();
//    deltaIcePrimary_.correctBoundaryConditions();
    pointScalarField deltaIcep = vpi_.interpolate(deltaIce_);
    forAll(intCoupledPatchIDs(), i)
    {
      const label intFilmPatchID=intCoupledPatchIDs()[i];
      const label filmPatchID=primaryPatchIDs()[i];

      scalarField deltaIce=deltaIcep.boundaryField()[intFilmPatchID].patchInternalField();
//      toPrimary(intFilmPatchID, deltaIce);
	const mappedWall& mpb = 
	    refCast<const mappedWall>
	    (
		pointMesh::New(regionMesh()).boundary()[intFilmPatchID]
	    );
	mpb.reverseDistribute(deltaIce);
      deltaIcePrimary_.boundaryFieldRef()[filmPatchID]=deltaIce;
//      Info<<"0"<<endl<<deltaIce_<<endl;
//      Pout<<"1"<<deltaIcep.boundaryField()[intFilmPatchID].patchInternalField()<<endl;
      Pout<<"2"<<endl<<deltaIce<<endl;
//      Pout<<"3"<<endl<<deltaIcePrimary_.boundaryField()[filmPatchID].patchInternalField()<<endl;
    }
*/
}

void iceSingleLayer::solveEnergy()
{
    if (debug)
    {
        InfoInFunction << endl;
...
//        this->mappedFieldAndInternalPatchTypes<scalar>()
    ...
    U8LWC("U8LWC", dimMass/dimArea/dimTime, readScalar(coeffs_.lookup("U8LWC"))),
    deltaIcePrimary_
    (
        IOobject
        (
        ...
        pointMesh::New(primaryMesh()),
        dimensionedScalar(deltaIce_.dimensions(), Zero)
    ),
    vpi_(regionMesh())
{
...
void iceSingleLayer::addSources
(
    const label patchi,
    const label facei,
...
void iceSingleLayer::preEvolveRegion()
{
    if (debug)
    {
        InfoInFunction << endl;
...
void iceSingleLayer::evolveRegion()
{
    if (debug)
    {
        InfoInFunction << endl;
...
const volScalarField& iceSingleLayer::Cp() const
{
    return Cp_;
}


const volScalarField& iceSingleLayer::kappa() const
{
    return kappa_;
}


const volScalarField& iceSingleLayer::T() const
{
    return T_;
}


const volScalarField& iceSingleLayer::Ts() const
{
    return Ts_;
}


const volScalarField& iceSingleLayer::Tw() const
{
    return Tw_;
}


const volScalarField& iceSingleLayer::hs() const
{
    return hs_;
}

const volScalarField& iceSingleLayer::deltaIce() const
{
    return deltaIce_;
}

const pointScalarField& iceSingleLayer::deltaIcePrimary() const
{
    return deltaIcePrimary_;
}

const volScalarField& iceSingleLayer::betta() const
{
    return betta_;
}

void iceSingleLayer::setU8LWC(const dimensionedScalar aU8LWC)
{
    U8LWC=aU8LWC;
}

void iceSingleLayer::info()
{
...
tmp<volScalarField::Internal> iceSingleLayer::Srho() const
{
...
tmp<volScalarField::Internal> iceSingleLayer::Srho
(
    const label i
) const
{
...
tmp<volScalarField::Internal> iceSingleLayer::Sh() const
{
