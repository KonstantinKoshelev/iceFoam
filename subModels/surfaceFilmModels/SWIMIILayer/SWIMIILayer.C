...
/*
    ...
    iceSp_ = chi / zna;*/
    volScalarField r = htcs_->h() * (TPrimary_ - T_);
/*    forAll(r, i)
    {
//	if ((deltaIce_[i] < 0.000001) && (delta_[i] < 0.00001))
	if ((deltaIce_[i] < 0.000001) && (delta_[i] < 1e-7))
	{
	  r[i] = 0.0;
	}
    }
...
//	+ (q(hs_) & hs_) 
    ...
    (hs_+Lf_)
    ;
    const scalar rdt = 1.0/time_.deltaTValue();
    forAll(iceSp_, i)
    {
/*	if ((deltaIce_[i]>0.001) && (delta_[i]<1e-7))
//	if ((betta_[i]<SMALL) && (delta_[i]<SMALL))
	{
	    iceSp_[i] = rhoSp_[i];
	}
*/
	const scalar maxiceSp_ = rhoSp_[i] - deltaRho_[i] * rdt;
	if (iceSp_[i] < maxiceSp_)
	    iceSp_[i] = rhoSp_[i]; //maxiceSp_;
	if (-iceSp_[i] < SMALL)
	    iceSp_[i] = 0.0;
    }

//    iceSp_.max(rhoSp_ - deltaRho_ * rdt);
    iceSp_.min(0.0);
/*
