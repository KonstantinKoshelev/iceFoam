...
)
{
	...
	//Info << "shearStress " << shearStress << endl;
...
void Myers::solveEnergy()
{
    ...
    volScalarField q_0 = (
			    - hsSp_
			    - rhoSp_ * Cp_ * TPrimary_//TParcels_
			    + htcs_->h() * TPrimary_
			) / kappa_ ;
    ...
    //Info << "Tw_" << Tw_[20] << endl;

    //Info << "deltaIce_" << deltaIce_[20] << endl;
    
	Ts_ = Tw_ + (q_0r + q_1r * Tw_) / (1.0 - q_1r * deltaIce_) * deltaIce_;
  
	
	volScalarField Twater = Tf_ + (q_0 + q_1 * Tf_ ) / (1.0 - q_1 * delta_) * delta_;
    
    
    volScalarField Wh = pos(pos0(Ts_ - Tf_) + alpha_);
	...
	(
		...
		- Wh * 
			(
				kappaIce_ * (Tf_ - Tw_) / (deltaIce_ * Lf_)
